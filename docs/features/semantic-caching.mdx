...
1. Open Bifrost UI at `http://localhost:8080` and go to **Settings > Caching**.
2. In the **Vector Store** card, toggle the switch to enable it.
3. Select a provider (e.g. **Weaviate**) from the dropdown and fill in the provider-specific fields.
4. Click **Save**. A restart of the Bifrost server is required for changes to take effect.

The Web UI supports all four vector store providers — Redis/Valkey, Weaviate, Qdrant, and Pinecone — with provider-specific configuration fields including TLS, authentication, and connection options.
...
> **UI Note**: The Web UI configures provider-backed semantic caching. If you want direct-only mode (`dimension: 1` with no `provider`), configure it through `config.json`. See [Direct Hash Mode](#direct-hash-mode-embedding-free) for details.
...
1. **Configure the Vector Store first**
   - Open Bifrost UI at `http://localhost:8080` and go to **Settings > Caching**.
   - In the **Vector Store** card at the top of the page, enable and configure a vector store provider (see [Vector Store Setup](#vector-store-setup)).
   - Save and restart Bifrost so the vector store is connected.

2. **Configure Semantic Cache Plugin**
   - Once the vector store is connected, the **Enable Semantic Caching** section becomes available below the vector store card.
   - Toggle the switch to enable it, and fill in the required fields.

**Required Fields:**
- **Provider**: Select from your configured providers (keys are inherited automatically).
- **Embedding Model**: The embedding model to use for caching (e.g. `text-embedding-3-small`).
...
<Info>
**Web UI**: Direct hash mode is not configurable through the Web UI. The UI is designed for provider-backed semantic caching. If you load a direct-only config via `config.json`, the UI will display a warning banner and allow you to view the settings, but you should continue using `config.json` to manage direct-only mode.
</Info>
